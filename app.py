...
def get_supabase():
    try:
        ...
        st.error("La connexion à la base de données EcoClean est indisponible.")
...
def load_signalements():
    try:
        response = supabase.table("signalements").select("*").order("id", desc=False).execute()
        rows = response.data or []
        if not rows:
            return pd.DataFrame(columns=["id", "type", "statut", "lat", "lng", "adresse", "date", "description"])
        ...
        return df[["id", "type", "statut", "lat", "lng", "adresse", "date", "description"]]
    except Exception as e:
        st.error("Impossible de charger les signalements.")
        st.caption(f"Détail technique : {e}")
        return pd.DataFrame(columns=["id", "type", "statut", "lat", "lng", "adresse", "date", "description"])
...
df_all = load_signalements()

if menu == PAGES[0]:
    total, pending, done, rate = counts(df_all)
    ...
    with f2:
        type_filter = st.multiselect("Type de déchet", sorted(df["type"].dropna().unique()) if not df.empty else [])
    with f3:
        secteur_filter = st.multiselect("Secteur", sorted(df["adresse"].dropna().unique()) if not df.empty else [])
    if statut_filter:
    ...
    if df.empty:
        st.caption("Aucun signalement ne correspond aux filtres sélectionnés.")
    for _, row in df.iterrows():
            ...
            with col3:
                if row["statut"] == "À ramasser":
                    if st.button("Marquer comme ramassé", key=f"ramassage_{int(row['id'])}", use_container_width=True):
                        try:
                            supabase.rpc("marquer_ramasse", {"p_id": int(row["id"])}).execute()
                            st.success("Signalement marqué comme ramassé.")
                            st.rerun()
                        except Exception as e:
                            st.error("Impossible de mettre à jour ce signalement.")
                            st.caption(f"Détail technique : {e}")
                else:
    ...
    with st.form("form_signalement", clear_on_submit=True):
        ...
        a, b = st.columns(2)
        with a:
            lat = st.number_input("Latitude", 43.50, 43.70, float(COLOMIERS_COORDS[0]), format="%.5f")
        with b:
            lng = st.number_input("Longitude", 1.20, 1.50, float(COLOMIERS_COORDS[1]), format="%.5f")
    ...
    if submitted:
        ...
        try:
            supabase.table("signalements").insert(payload).execute()
            st.success("Votre signalement a été ajouté à la carte.")
            st.rerun()
        except Exception as e:
            st.error("Impossible d'ajouter le signalement.")
            st.caption(f"Détail technique : {e}")
    ...
    if df.empty:
        st.caption("Aucun signalement enregistré pour le moment.")
    else:
        chart = df["statut"].value_counts().rename_axis("Statut").to_frame("Nombre")
        st.bar_chart(chart)
    st.caption("Les statistiques sont calculées à partir des signalements enregistrés dans la base de données EcoClean.")
