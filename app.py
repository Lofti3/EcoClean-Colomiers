...
st.set_page_config(page_title="EcoClean Colomiers", page_icon="🌱", layout="wide", initial_sidebar_state="expanded")

COLOMIERS_COORDS = [43.6135, 1.3330]
PAGES = ["🏠 Accueil", "🗺️ Carte des déchets", "📸 Signaler un déchet", "📊 Impact citoyen"]
...
.subtitle {font-size:1.05rem;color:#667085;margin-top:.2rem}
</style>
""", unsafe_allow_html=True)

DEMO_DATA = [
    {"id":1,"type":"🧃 Gourde de compote / Emballage gâteau","statut":"🔴 À ramasser","lat":43.6145,"lng":1.3325,"adresse":"Près du Parc Duroch","date":"2026-09-01","description":"Déchet visible dans l'espace public."},
    ...
    {"id":3,"type":"🚬 Mégots groupés","statut":"🔴 À ramasser","lat":43.6162,"lng":1.3352,"adresse":"Centre-ville / Vieux Colomiers","date":"2026-09-05","description":"Plusieurs mégots regroupés."},
    {"id":4,"type":"🍬 Emballage de bonbon / Papiers","statut":"🔴 À ramasser","lat":43.6122,"lng":1.3295,"adresse":"Secteur du Pigeonnier","date":"2026-09-06","description":"Petits emballages abandonnés."},
]
...
def build_map(df):
    ...
    folium.Marker(COLOMIERS_COORDS, tooltip="🌱 EcoClean Colomiers", popup="Plateforme citoyenne de Colomiers", icon=folium.Icon(color="blue", icon="home", prefix="fa")).add_to(m)
    for _, row in df.iterrows():
        color = "red" if row["statut"] == "🔴 À ramasser" else "green"
        popup = f"<div style='width:250px;font-family:Arial'><h4>{row['type']}</h4><b>Statut :</b> {row['statut']}<br><b>Lieu :</b> {row['adresse']}<br><b>Date :</b> {row['date']}<br><b>Description :</b> {row.get('description','')}<br><b>ID :</b> #{int(row['id'])}</div>"
...
# Navigation via URL : le bouton d'accueil peut donc ouvrir directement la carte.
...
st.markdown('<div class="subtitle">Une plateforme citoyenne pour signaler, ramasser et réduire les micro-déchets à Colomiers.</div>', unsafe_allow_html=True)
...
# Si l'utilisateur choisit une page dans la sidebar, on synchronise l'URL.
selected_key = next(key for key, value in PAGE_KEYS.items() if value == menu)
if selected_key != page_key and page_key != "home":
...
if menu == PAGES[0]:
    ...
    st.write("Vous voyez un déchet abandonné ? Signalez-le. Vous passez près d'un déchet signalé ? Vous pouvez le ramasser et contribuer à améliorer votre ville.")
    ...
    with left:
        ...
        st.markdown("**1. 📸 Signaler** — vous trouvez un déchet et le signalez.  \n**2. 🗺️ Localiser** — il apparaît sur la carte.  \n**3. 🧤 Ramasser** — n'importe quel citoyen peut agir.  \n**4. ✅ Confirmer** — cliquez sur « Je l'ai ramassé »." )
    with right:
        st.markdown("### 🌱 Notre objectif")
        st.info("Une personne signale, une autre peut agir. EcoClean transforme les petits gestes en impact collectif.")
    st.markdown("### 🧤 Vous avez envie d'agir ?")
    st.write("Cliquez ci-dessous : vous serez dirigé directement vers la carte des déchets à ramasser.")
    if st.button("🗺️ Voir les déchets à ramasser", use_container_width=True, type="primary"):
    ...
    with f1:
        statut_filter = st.multiselect("Statut", ["🔴 À ramasser","🟢 Ramassé"], default=default_status)
    with f2:
        type_filter = st.multiselect("Type de déchet", sorted(df["type"].unique().tolist()))
    with f3:
        secteur_filter = st.multiselect("Secteur", sorted(df["adresse"].unique().tolist()))
    ...
    if df.empty:
        ...
        for index,row in df.iterrows():
            ...
            with col1: st.markdown(f"**#{int(row['id'])} — {row['type']}**  \n📍 {row['adresse']} · 📅 {row['date']}")
            with col2: st.write(f"Statut : **{row['statut']}**")
            with col3:
    ...
    st.write("Vous avez repéré un déchet dans l'espace public ? Signalez-le pour que les autres citoyens puissent le voir.")
    with st.form("form_signalement", clear_on_submit=True):
        col1,col2 = st.columns(2)
        with col1:
            type_dechet = st.selectbox("🗑️ Type de déchet", ["🧃 Gourde de compote / Emballage gâteau","🥤 Canette / Bouteille plastique","🚬 Mégots groupés","🍬 Emballage de bonbon / Papiers","♻️ Autre déchet recyclable","🗑️ Autre déchet"])
        with col2:
    ...
    if submitted:
        ...
        new_row = {"id":new_id,"type":type_dechet,"statut":"🔴 À ramasser","lat":lat,"lng":lng,"adresse":secteur,"date":datetime.now().strftime("%Y-%m-%d"),"description":description or "Aucune description."}
        st.session_state.dechets_db = pd.concat([db,pd.DataFrame([new_row])], ignore_index=True)
...
st.markdown("---")
st.caption("🌱 EcoClean Colomiers — Une initiative citoyenne pour une ville plus propre.")
